...
const _ServerSideWebsite = class extends import_abstracts.AwsConstruct {
  ...
  async uploadAssets() {
    (0, import_logger.log)(`Deploying the assets for the '${this.id}' website`);
    ...
    for (const [pattern, filePath] of Object.entries(this.getAssetPatterns())) {
      ...
      if (fs.lstatSync(filePath).isDirectory()) {
        (0, import_logger.log)(`Uploading '${filePath}' to 's3://${bucketName}/${s3PathPrefix}'`);
        ...
        (0, import_logger.log)(`Uploading '${filePath}' to 's3://${bucketName}/${targetKey}'`);
  ...
  async preRemove() {
    ...
    (0, import_logger.log)(`Emptying S3 bucket '${bucketName}' for the '${this.id}' website, else CloudFormation will fail (it cannot delete a non-empty bucket)`);
...
ServerSideWebsite.commands = {
  "assets:upload": {
    ...
    handler: _ServerSideWebsite.prototype.uploadAssets
  }
};
