...
const _Queue = class extends import_abstracts.AwsConstruct {
  constructor(scope, id, configuration, provider) {
    ...
    if (configuration.delay !== void 0) {
      ...
      delay = import_core.Duration.seconds(configuration.delay);
    }
    ...
    this.configuration.worker.events = [
      {
        sqs: {
          ...
          batchSize,
          maximumBatchingWindow: this.queue.fifo ? void 0 : 60
        }
      }
    ];
  ...
  async listDlq() {
    ...
    if (dlqUrl === void 0) {
      console.log(import_chalk.default.red('Could not find the dead letter queue in the deployed stack. Try running "serverless deploy" first?'));
      return;
    }
    const progress = (0, import_ora.default)("Polling failed messages from the dead letter queue").start();
    ...
    for (const message of messages) {
      console.log(import_chalk.default.yellow(`Message #${(_a = message.MessageId) != null ? _a : "?"}`));
      console.log(this.formatMessageBody((_b = message.Body) != null ? _b : ""));
      console.log();
    }
  ...
  async purgeDlq() {
    ...
    const progress = (0, import_ora.default)("Purging the dead letter queue of failed messages").start();
    await this.provider.request("SQS", "purgeQueue", {
      QueueUrl: dlqUrl
    });
    await (0, import_sleep.sleep)(500);
    progress.succeed("The dead letter queue has been purged, failed messages are gone \u{1F648}");
  }
  async retryDlq() {
    const queueUrl = await this.getQueueUrl();
    const dlqUrl = await this.getDlqUrl();
    if (queueUrl === void 0 || dlqUrl === void 0) {
      console.log(import_chalk.default.red('Could not find the queue in the deployed stack. Try running "serverless deploy" first?'));
      return;
    }
    const progress = (0, import_ora.default)("Moving failed messages from DLQ to the main queue to be retried").start();
    ...
    progress.succeed(`${totalMessagesRetried} failed message(s) moved to the main queue to be retried \u{1F4AA}`);
  }
  ...
  displayLogs(options) {
    ...
    console.log(import_chalk.default.gray(`serverless ${args.join(" ")}`));
