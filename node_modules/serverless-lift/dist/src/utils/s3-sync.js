...
async function s3Sync({
  ...
  for (const batch of (0, import_lodash.chunk)(filesToUpload, 2)) {
    await Promise.all(batch.map(async (file) => {
      const targetKey = targetPathPrefix !== void 0 ? path.join(targetPathPrefix, file) : file;
  ...
  const targetKeys = filesToUpload.map((file) => targetPathPrefix !== void 0 ? path.join(targetPathPrefix, file) : file);
